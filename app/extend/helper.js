...
module.exports = {
        ...
        if (subReleases && validator.isBase64(subReleases)) {
            try {
                ...
                if (pageBuildSubReleases.length) {
                    const releaseMap = await ctx.curlIntranetApi(`${ctx.webApi.releaseInfo}/list?releaseIds=${pageBuildSubReleases.map(x => x.releaseId).toString()}&projection=resourceType`)
    ...
    async convertTestNodePageBuild(template, pageBuildStr, nodeInfo, userId, subReleases) {
